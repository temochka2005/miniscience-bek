...
class OkushiriTsunami(Application):
    def add_user_options(self, group):
        ...
        group.add_argument(
            "--dx", action="store", type=float, dest="dx", default=0.019,
            help="Spacing between the particles")
        group.add_argument(
            "--dw0", action="store", type=float, dest="dw0", default=0.135,
            help="Initial depth of the fluid column (m)")
        group.add_argument(
            "--le", action="store", type=float, dest="le", default=5.5,
            help="Length of the fluid domain (m)")
        group.add_argument(
            "--w", action="store", type=float, dest="w", default=3.40,
            help="Width of the fluid domain (m)")
        group.add_argument(
            "--n", action="store", type=float, dest="n", default=0.025,
            help="Manning coefficient ")
        group.add_argument(
            "--Vb", action="store", type=float, dest="Vb", default=1.96e-3,
            help="Volume of the bed particles")
        group.add_argument(
            "--hb", action="store", type=float, dest="hb", default=1.0e-2,
